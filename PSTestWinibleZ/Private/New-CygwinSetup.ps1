Function New-CygwinSetup {
    ...
    # pynacl takes a while to install, need to find a better way to speed that
    # up
    Write-Verbose -Message "Installing the required Python modules for Ansible/WinRM"
    $rc = Invoke-BashCygwin -Executable $bash_exe -Arguments "LIBSODIUM_MAKE_ARGS=-j4 pip2 install ansible pywinrm[credssp] virtualenv"
    if ($rc -ne 0) {
